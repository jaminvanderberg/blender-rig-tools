...
from rigtools.rotation_follow import create_rotation_follow_setup
...
class RIG_OT_create_fk_tweak_chain(bpy.types.Operator):
	...
	mch_bone_template: bpy.props.StringProperty(
		name="MCH Bone Name",
	...
	def create_tweak_chain(self, armature_data, chain_bone_names) -> FKTweakChain :
		...
		for bone_name in chain_bone_names:
			...
			if self.skip_first_tweak and bone_name == chain_bone_names[0]:
	...
	def draw(self, context):
		...
		if self.create_rotation_follow_setup:
			...
			col.prop(self, "mch_bone_template")
