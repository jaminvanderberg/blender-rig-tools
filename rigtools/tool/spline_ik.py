...
class SplineIK:
	...
	def edit_mode(self, context, mch_bone_names, name_source=None):
		...
		for i in range(self.control_count):
			chain_pos = (i / (self.control_count - 1)) * bone_count
			...
			if i == 0:
				pos_name = "start"
			elif i == self.control_count - 1:
				pos_name = "end"
			elif i == 1 and self.control_count == 3:
				pos_name = "mid"
			else:
			...
			# Twist controllers
			do_twist = False
			if self.twist_type == 'ALL':
				do_twist = True
			elif i == 0 and self.twist_type in ['START', 'START_END']:
				do_twist = True
			elif i == self.control_count - 1 and self.twist_type in ['END', 'START_END']:
				do_twist = True
			...
			if self.ik_collection_name:
				set_bone_collection(obj.data, spline_bone, self.ik_collection_name)
				if twist_bone:
					set_bone_collection(obj.data, twist_bone, self.ik_collection_name)
			else:
			...
			self.control_names.append(spline_bone.name)

		if self.twist_names and self.twist_type in ['START', 'START_END', 'ALL']:
			first = edit_bones[self.twist_names[0]]
	...
	def object_mode(self, context):
		...
		spline.bezier_points.add(len(self.control_names) - 1)
		...
		for i, control_name in enumerate(self.control_names):
			...
			if i == 0:
				next = obj.data.bones[self.control_names[i + 1]].head_local.copy()
			...
			elif i == len(self.control_names) - 1:
				prev = obj.data.bones[self.control_names[i - 1]].head_local.copy()
	...
	def pose_mode(self, context):
		...
		spline_constraint.target = self.spline_object
		spline_constraint.chain_count = len(self.mch_bone_names) - 1 if self.skip_first else len(self.mch_bone_names)

		ik_names = self.mch_bone_names[1 if self.skip_first else 0:]

		# Twist
		names = self.twist_names or []
